...
import { mockProducts, mockRequirements, mockEnquiries } from "@/data/mockData";
import { Product, IntentLevel, BuyerRequirement } from "@/types/marketplace";
import { Search, Plus, X, FileText, ShoppingBag } from "lucide-react";
...
export function BuyerDashboard() {
  ...
  const handleEnquirySubmit = (message: string, intentLevel: IntentLevel) => {
    console.log('Enquiry submitted:', { product: selectedProduct?.name, message, intentLevel });
  ...
  const handlePostRequirement = () => {
    if (!newRequirement.productNeeded.trim()) return;
    ...
    setNewRequirement({ productNeeded: '', quantity: '', location: '', timeline: '', description: '' });
  ...
  return (
    <div className="min-h-screen bg-background">
            ...
            {isPostingRequirement && (
              <Card className="mb-6 animate-fade-in">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
                  ...
                  <Button variant="ghost" size="icon" onClick={() => setIsPostingRequirement(false)}>
                    <X className="w-4 h-4" />
                  </Button>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => setNewRequirement({ ...newRequirement, productNeeded: e.target.value })}
                      />
                    </div>
                    <div className="space-y-2">
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    <Button onClick={handlePostRequirement} disabled={!newRequirement.productNeeded.trim()}>
                      Post Requirement
                    </Button>
                  </div>
                </CardContent>
      ...
      <ClosureFeedbackModal
        ...
        onSubmit={(reason) => {
          console.log('Closure feedback:', reason);
          toast({
